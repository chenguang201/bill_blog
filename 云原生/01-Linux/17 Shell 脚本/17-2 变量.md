...
```powershell
$n  	#(功能描述:n 为数字，$0代表该脚本名称，$1-$9 代表第一到第九个参数，十以上的参数，十以上的参数需要用大括号包含，如${10}) 
```
...
```powershell
#!/bin/bash
# 要加入路由的ip 数组，以空格分开，请用自己需要访问的目标服务器ip替换
# 定义一个数组
...
if [ ! -n "$gw" ]; then
  ...
  #
	for ip in ${ips[*]}
	do
		sudo route add -net $ip -netmask 255.255.0.0 $gw && \
		echo "add route: ip: $ip, gateway: $gw" || \
		echo "add route fialed."
	done
fi
```

在Shell脚本中，`${ips[*]}`  是一个数组扩展语法，它会将数组 ips 中的所有元素合并成一个字符串，每个元素之间用空格分隔。

例如，如果 ips 数组包含两个元素 124.70.129.0 和 124.70.128.0，那么 `${ips[*]}` 扩展后的结果是 `"124.70.129.0 124.70.128.0"` 。这个字符串可以在循环或命令行中使用，如在代码中的第10行，`for ip in ${ips[*]}`，表示循环遍历数组 ips 中的所有元素。
