...
# 414. 当访问这个入口的时候，可以发现后面有三个pod的服务在等待调用，
# 415. kube-proxy会基于rr（轮询）的策略，将请求分发到其中一个pod上去
# 416. 这个规则会同时在集群内的所有节点上都生成，所以在任何一个节点上访问都可以。
...
**<font style="color:#E8323C;">userspace 模式</font>**  
...
# 420. 为了方便后面的测试，修改下三台nginx的index.html页面（三台修改的IP地址不一致）
# 421. kubectl exec -it pc-deployment-66cb59b984-8p84h -n dev /bin/sh
# 422. echo "10.244.1.40" > /usr/share/nginx/html/index.html
...
# 425. 查看service的详细信息
# 426. 在这里有一个Endpoints列表，里面就是当前service可以负载到的服务入口
...
# 431. 修改分发策略----sessionAffinity:ClientIP

# 432. 查看ipvs规则【persistent 代表持久】
[root@master ~]# ipvsadm -Ln
...
在之前的样例中，创建的Service的ip地址只有集群内部才可以访问，如果希望将Service暴露给集群外部使用，那么就要使用到另外一种类型的Service，称为NodePort类型。NodePort的工作原理其实就是**将service的端口映射到Node的一个端口上**，然后就可以通过`<font style="color:#E8323C;">NodeIp:NodePort</font>`来访问service了。  
![](images/47.png)
...
# 441. 接下来可以通过电脑主机的浏览器去访问集群中任意一个nodeip的30002端口，即可访问到pod
```
